...
const vendorSearch = async (req, res) => {
  ...
  try {
    ...
    const totalCountPromise = Vendor.countDocuments(query); // Находим кол-во результатов
    const searchPromise = vendorSearchQuery({ query, page, limit }); // Находим результат
    ...
    res.status(200).json(message.success('VendorSearch ok', result));
  } catch (error) {
    const analyticsId = analytics('VENDOR_SEARCH_ERROR', {
      error,
      body: req.body,
      entity: 'Vendor',
      user: userId,
    ...
    res.status(400).json(message.fail('VendorSearch error', analyticsId));
  }
};

export default vendorSearch;

function vendorSearchQuery({ query, page, limit }) {
  return Vendor.find(query)
    ...
    .exec()
    .then((docs) => {
      return message.success('Vendor found', docs);
    });
}
